...
def save_results(logits_matrix, targets_list, epoch, args):
    """
    Saves the predicted logits matrix, true labels, sample ids and class
    dictionary for further analysis of results
    """
    ...
    if not os.path.exists(path):
        os.mkdir(path)
    path_to_save = os.path.join(
        args.ckpt, args.logname + '_e'+str(epoch) + "_test_results.pkl")
...
def weightCrossEntropyLoss(feature_comp, complabel, w_vt, w_pt):
    ...
    for b in range(size):
        ...
        result_s = 0-feature[b][complabel[b]] + math.log(sum_s)
...
def rownorm(weight):
    ...
    for i in range(len(weight)):
        Zmax, Zmin = max(weight[i]), min(weight[i])
        weight[i] = (weight[i] - Zmin) / (Zmax - Zmin)
    return weight