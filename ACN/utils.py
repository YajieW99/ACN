...
def save_results(logits_matrix, targets_list, args, epoch):
    """
    ...
    path_to_save = os.path.join(
        args.ckpt, args.logname + '_e' + str(epoch) + '_' + "test_results.pkl")

    with open(path_to_save, "wb") as f:
        pickle.dump([logits_matrix,targets_list], f)
...
def weightCrossEntropyLoss(feature_comp, complabel, w_vt, w_pt):
    '''
    Computes the weighted crossentropyloss, return the loss without means
    :param feature_comp: [N_comp,174]
    :param complabel:    [N_comp]
    :param w_vt:         [N_comp]
    :param w_pt:        [N_comp]
    :return:
    '''
    size = feature_comp.shape[0]
    softmax = torch.nn.Softmax(dim=0)
    loss_w_add = 0
    feature = []

    for b in range(size):
        currow = feature_comp[b]
        currow = softmax(currow)
        feature.append(currow)
    for b in range(size):
        sum_s = 0
        for i in range(16):
            sum_s = sum_s + math.exp(feature[b][i])
        result_s = 0.000-feature[b][complabel[b]] + math.log(sum_s)
        weight = w_vt[b]*w_pt[b]
        result_s = weight*result_s
        loss_w_add = loss_w_add + result_s
    return loss_w_add/size


def clsweightCrossEntropyLoss(cls, label, clsweight):
    size = cls.shape[0]
    softmax = torch.nn.Softmax(dim=0)
    loss_w_add = 0
    feature = []
    for b in range(size):
        currow = cls[b]
        currow = softmax(currow)
        feature.append(currow*clsweight)
    for b in range(size):
        sum_s = 0
        for i in range(16):
            sum_s = sum_s + math.exp(feature[b][i])
        result_s = 0.000-feature[b][label[b]] + math.log(sum_s)
